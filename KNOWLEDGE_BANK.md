...
## §33 — 2026-06-12 — Part-3 story / artwork mismatch (BLOOM-2026-A & -D 《好长好长》): backend silent fallback + frontend one-off heal

### Symptom

Two independent pilot teachers (`BLOOM-2026-A` on 2026-06-12 morning;
`BLOOM-2026-D` reported the same the day before) opened《好长好长》
(g2v2-u4-l4), navigated to Part 3, picked / uploaded the 《小真的
长头发》 illustration, generated a story — and got back a 100 %
《桃花源》-themed story (溪水 / 竹排 / 桃花林 / 老渔人) attached
to the 小真的长头发 picture. The 故事 ✓ and 动画 ✓ completion
badges both showed up so the model was clearly *finishing* the
generation; the content was just talking about the wrong artwork.

Two teachers independently producing the exact same symptom on the
same artwork on the same lesson was the immediate signal that this
is structural, not user error.

### Root cause (one bug, two collaborating layers)

**Backend — silent `artwork_id` fallback.**
`backend/lesson_context.py` line 136 was:

```python
chosen = artwork_id or seed.default_executor_b_artwork_id
```

For U4-L4 the LKP has `default_executor_b_artwork_id: "G2V2-U4-L4-art01"`
(桃花源1). When the frontend request body's `artwork_id` was falsy
(`None`, empty, missing), the backend silently picked art01.
`_build_story_lesson_context` then dropped that artwork's
visual description / teacher_guide_notes / story_hint straight into
the `[本画作]` system-prompt block — i.e. the model saw a strong
**text** signal saying "this picture is 桃花源1" while the user
message's base64 was actually 小真的长头发. With Doubao Vision,
text grounding usually wins; out came a 桃花源 story.

The fallback was added for legacy back-compat (single-artwork lessons
where `artwork_id` was nice-to-have). On multi-artwork lessons it's
silently wrong.

**Frontend — `selectedArtworkId` is null whenever the teacher
uploads.** `setArtworkFromUrl(url, artworkId)` (curated path) sets
`pair.selectedArtworkId = artworkId` and the request sends
`artwork_id`. But `addUploadedArtwork(dataUrl)` (file-picker path)
sets `pair.selectedArtworkId = null, pair.selectedUploadedId = id`
— so the request body's `artwork_id` is `undefined` → JSON.stringify
omits it → backend hits the fallback.

The two layers conspire: any teacher who opens U4-L4 Part 3 and
clicks the centre canvas's "上传图片" button to upload a copy of one
of the textbook artworks (instead of clicking the sidebar's curated
thumbnail) gets a story about 桃花源1 stamped on her uploaded
image. The UX trap is in the upload button being visually as
prominent as the curated thumbnails, and human reflex is "I have
the image, let me upload it" rather than scrolling the sidebar to
look for it.

### Fix — two layers, no schema changes

This PR ships **B + A together** (per the user's explicit ask), in
that order of impact.

#### B — Backend hardening (5-line guard + companion helper)

`backend/main.py` `_build_story_lesson_context()` now intercepts
multi-artwork lessons before the legacy fallback fires:
...
The new `_build_story_lesson_context_uploaded(seed)` helper builds
the same lesson-level pedagogy block (unit big idea / learning task
/ three-tier objectives / teaching focus + difficulty / key art
concepts / assessment criteria) but **deliberately omits** the
`[本画作]` block. In its place we drop a one-liner telling the
model:

> 「[本画作] 这是老师上传的学生 / 自有作品，**请只根据画面里你
> 实际看到的内容**来写故事；不要引用本课教材里任何一幅大师作品的名字、画家、画面描述或教参解读。把它当作一幅主题贴合
> "{learning_task_zh}" 的全新作品来对待。」

The explicit "don't name any masterpiece from the textbook" line is
load-bearing: without it the model still latches on to the
lesson-level pedagogy and free-associates a story about one of the
textbook artworks anyway. We tested several gentler wordings during
draft; only the explicit-prohibition version produced
visually-grounded stories on uploaded photos.

Single-artwork lessons (which currently means none, but the schema
allows it) fall through to the legacy path — the original
`artwork_id or default_executor_b_artwork_id` fallback is the
correct behaviour for them. `build_executor_b_context` and
`build_executor_c_context` in `lesson_context.py` are unchanged;
the defensive routing happens entirely in `main.py` because that's
where `StoryRequest.artwork_id` lives.

The animation endpoint (`/api/animation/submit` →
`build_executor_c_context`) is **not** hardened in this PR. The
animation prompt is much more visual-only (`animation_brief` +
mood) and the visible failure mode there is way less catastrophic
than story-text mismatches. If a similar incident shows up in
animation we'll port the same pattern.

#### A — Frontend one-off self-heal (BLOOM-2026-A / -D only)

`frontend/src/stores/projects.ts` gets a new
`healPart3Mismatch(p, inviteCode)` helper alongside the existing
`healStuckProject` (§31). It runs inside `migrateProjects()` which
already executes on both hydration paths (localStorage boot AND
`loadFromAPI()` post-login).

Scope guards (both must match or it's a no-op):
- `inviteCode ∈ {'BLOOM-2026-A', 'BLOOM-2026-D'}`
- `p.meta.lessonId === 'g2v2-u4-l4'`

For matched projects, it wipes every `part3Snapshot.pairs[i]`'s
generation-related state AND uploaded-artwork state:
- `storyData`, `storyError`, `storyStreamText`, `storyLoading`
- `animationVersions`, `animationError`, `animationLoading`,
  `remainingAttempts → 3`, `chosenVideoUrl`
- `selectedChoiceId`, `generatedContinuations`,
  `continuationStreamText`, `continuationLoading`,
  `continuationError`
- `designChatMessages`, `animationChatMessages`
- **`artworkStates: {}`** + `activeArtworkKey: null`
- **`selectedArtworkId: null`** + `selectedUploadedId: null` +
  **`uploadedArtworks: []`** (deliberately also clears the
  teacher's prior uploaded copies — see below for why)
- `imageDataUrl / imageUrl / imageBase64: null`

Idempotency: a `allClean` check at the top no-ops when every pair
is already in the swept state, so the heal doesn't keep firing
each session after the teacher has cleanly re-generated. The first
clean autosave PUT after the heal writes the cleared state to the
server; subsequent reloads see it idempotent.

Invite code is read directly from `localStorage['artbloom-username']`
(the legacy key that the user store persists invite code under).
That's a one-line cheap read — avoids importing `useUserStore()`
into the projects store, which would create a load-order issue
because `useProjectsStore()` itself initialises at app boot before
the user store's reactive refs are necessarily settled.

### Why wipe `uploadedArtworks: []` along with the rest

The naive heal would only clear `storyData` / `animationVersions`
and leave the teacher's previously-uploaded 小真的长头发 image
intact. But that path leaves the teacher staring at the same
"upload" thumbnail in the sidebar; her reflex on opening the
session will be to click that thumbnail and regenerate — which
sets `selectedArtworkId = null` AGAIN and re-triggers the same
wrong-artwork path even though backend B is now in place (because
B's fix doesn't reverse the policy — it makes uploaded artworks
get a *generic* prompt, but those generic prompts still aren't
"the 小真的长头发 textbook entry"). The teacher would then call us
saying "the new story is also wrong, it's about a generic
long-haired girl instead of *the actual textbook artwork*".

By also clearing `uploadedArtworks: []`, the sidebar shows only
the curated textbook thumbnails. The teacher's only path forward
is to click the curated 小真的长头发 thumbnail → `setArtworkFromUrl(
'/textbook-assets/.../art03-changtoufa.jpg', 'G2V2-U4-L4-art03')`
→ correct `artwork_id` over the wire → correct
`[本画作] 《小真的长头发》` block in the system prompt → correct
story. We force the safe rail.

### Files touched

- `backend/main.py` — new `_build_story_lesson_context_uploaded(seed)`
  helper + early-return branch in `_build_story_lesson_context()`.
- `frontend/src/stores/projects.ts` — `PART3_RESET_INVITE_CODES`,
  `PART3_RESET_LESSON_ID`, `_readInviteCodeFromLocalStorage()`,
  `healPart3Mismatch()`, integrated into `migrateProjects()` after
  the existing `healStuckProject` call.
...
- `python3 -c "import ast; ast.parse(open('backend/main.py').read())"` —
  exits 0.
- Manual smoke (local dev, mocked invite code in localStorage):
  1. Set `localStorage['artbloom-username'] = 'BLOOM-2026-A'`.
  2. Add a project with `meta.lessonId = 'g2v2-u4-l4'` and a
     fake `pair[0].storyData = { part1: 'fake 桃花源 story...' }`,
     `pair[0].uploadedArtworks = [{ id: 'ua-x', imageDataUrl: 'data:...' }]`.
  3. Reload page. After `migrateProjects()`, `pair[0].storyData ===
     null`, `pair[0].uploadedArtworks.length === 0`. ✓
  4. Pick the curated 小真的长头发 thumb in sidebar. Backend
     receives `artwork_id: 'G2V2-U4-L4-art03'`. Story prompt's
     `[本画作]` block now reads `《小真的长头发》` and the model
     generates a correctly-grounded story. ✓
  5. Upload your own random image (without picking a curated
     thumb). Backend receives no `artwork_id`. New branch fires:
     prompt's `[本画作]` block is the generic "teacher uploaded
     her own work" placeholder. Model writes a story that
     describes the actual uploaded image (no 桃花源 leak). ✓

### Notes for future maintainers

- **Don't relax the multi-artwork branch in `_build_story_lesson_context`.**
  The fallback to `default_executor_b_artwork_id` is provably
  unsafe whenever an LKP has 2+ textbook_artworks. If a future
  schema lets a teacher pick "no artwork, fully freeform", the
  cleanest extension is to add an explicit `is_uploaded_artwork:
  bool` flag to `StoryRequest` so we don't have to keep guessing
  from a null `artwork_id`. The current heuristic (no artwork_id +
  multi-artwork lesson → uploaded mode) is fine but won't survive
  a single-artwork lesson where the teacher still wants to upload
  her own image.
- **The `_build_story_lesson_context_uploaded` helper's
  "请只根据画面里你实际看到的内容" line is load-bearing.** Without
  the explicit "don't reference textbook masterpieces" forbiddance,
  Doubao Vision still drifts into describing one of the lesson's
  curated artworks. Don't soften this line.
- **The `healPart3Mismatch` invite-code list is intentionally a
  hardcoded `Set<string>` of two codes.** It's not driven by env
  vars or a database flag — by design. The heal is destructive
  (it wipes existing stories) and we never want it to be turned
  on remotely without a code change going through PR review.
  Once both teachers report a clean re-generation we can remove
  both codes and the helper becomes inert across all sessions.
- **`migrateProjects()` is the canonical hook for "fix-up at
  hydrate".** Both §31 (Part-6 stuck-on-converting) and §33
  (Part-3 story mismatch) live here because it's the only function
  invoked on both the localStorage init path AND the
  `loadFromAPI()` server-refresh path. If you add a third heal,
  follow the same pattern: a small `heal*` helper called from
  inside `migrateProjects().map(...)`, with both invite-code (or
  project-id) and lesson-id guards. Don't add heals inside
  `setActiveProject` — that runs once per session per project,
  not once per snapshot.
- **The animation endpoint (`build_executor_c_context`) has the
  same fallback pattern and could in principle exhibit the same
  bug.** It hasn't been reported because (a) animation prompt is
  much more visual-grounded, (b) `animation_brief` is shorter and
  more action-oriented so the textual pull is weaker. If a teacher
  ever reports "wrong animation for the artwork I picked" the fix
  is to port the same `_build_story_lesson_context_uploaded`
  pattern over for the C path.
- **CORS to `artbloomedu.com`** appeared again in the same
  screenshot. That's the §28 incident still not fully resolved on
  Render. See the operational follow-up there — likely the
  `CORS_ALLOW_ORIGINS` env var on Render is overriding the
  hardcoded floor or the backend wasn't redeployed after the
  §28 change. Out of scope for §33 (the heal fires locally and
  the autosave PUT to fix the cleared state can ride a later
  successful sync) but tracking as an independent op task.

### Operational follow-up

- After this PR deploys, ping BLOOM-2026-A and BLOOM-2026-D:
  1. Open `https://artbloomedu.com` once. The console will show
     `[projects] healing Part-3 story↔artwork mismatch on proj-…
     for inviteCode=BLOOM-2026-X; teachers will re-pick artwork
     and re-generate.`
  2. Navigate to Part 3. The image area + story panel should be
     empty (no ✓ badges).
  3. In the LEFT sidebar (under the 7 part list), find the three
     small thumbnails labelled 桃花源1 / 桃花源2 / 小真的长头发.
     Click one.
  4. Hit「生成故事」. Confirm the resulting story actually
     matches the artwork name (not a 桃花源 story under 小真的
     长头发).
  5. If she wants to use her own version of 小真的长头发: hit
     「上传图片」 (the central upload area). The story she gets
     back should now describe the actual photo she uploaded
     (generic-grounded), not 桃花源.
- Optionally tail the Render logs while she logs in — the new
  backend prints `[story] lesson_id=… artwork_id=…` calls (well,
  it doesn't yet — adding that print is a 1-line follow-up that
  was deferred from this PR's "5-line minimum" scope). If we want
  it, add `print(f"[story] lesson_id={req.lesson_id} artwork_id={
  req.artwork_id}", flush=True)` at the top of
  `_build_story_lesson_context`.

...
# ── Finalised story narration → Cloudflare R2 (public bucket) ────────
#
# Teachers can preview each half of the story through /api/tts above,
# but the deliverable they want to keep is a single stitched MP3 of
# the whole story (Part 1 + chosen continuation). This endpoint takes
# both halves, synthesises them as ONE edge-tts call (not two-and-
# concatenate — one call sounds better because the model gets full
# prosodic context across the paragraph join), and uploads the result
# to a public R2 bucket so the response URL is permanent + cacheable
# at Cloudflare's edge.
#
# Idempotent + content-hashed: the object key is
#     stories/{project_id}/{pair_id}/{sha256(voice|text)[:32]}.mp3
# so re-finalising the same (voice, text) tuple HEADs the existing
# object and returns the cached URL in ~30 ms — no re-synthesis,
# no re-upload, no edge-tts quota burned.

import hashlib
import re as _safe_id_re

import r2_client as _story_r2


class FinalizeStoryRequest(BaseModel):
    project_id:  str
    pair_id:     str       # = Part3Pair.id on the frontend
    voice_id:    str = "zh-CN-XiaoxiaoNeural"
    part1_text:  str
    part3_text:  str
...
def _safe_path_segment(value: str, max_len: int = 64) -> str:
    """
    Project / pair IDs come from client-supplied state. Restrict to
    a small alphabet so they can't escape the `stories/…` prefix or
    break R2 key constraints.
    """
    cleaned = _safe_id_re.sub(r"[^A-Za-z0-9._\-]", "", value)
    return cleaned[:max_len] or "default"


@app.post("/api/tts/finalize-story", response_model=FinalizeStoryResponse)
async def finalize_story_tts(req: FinalizeStoryRequest):
    """Synthesise + upload the full narration to public R2; return URL."""
    if not _story_r2.enabled():
        raise HTTPException(
            503,
            "Story-audio R2 bucket is not configured. Set "
            "R2_STORY_AUDIO_BUCKET + R2_STORY_AUDIO_PUBLIC_URL "
            "+ R2_ENDPOINT_URL / R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY "
            "in the backend environment.",
        )

    # Combine the two halves with a paragraph break so edge-tts inserts
    # a natural pause between Part 1 and Part 3. The frontend
    # `currentContentHash()` computes its hash from the same canonical
    # form so client-side staleness checks line up byte-for-byte.
    combined = f"{req.part1_text.strip()}\n\n{req.part3_text.strip()}".strip()
    if not combined:
        raise HTTPException(400, "Story text is empty — generate the story first.")
    if not req.part3_text.strip():
        raise HTTPException(
            400,
            "The second half of the story is empty. Pick an interactive "
            "choice in Story Preview and let it generate before finalising.",
        )

    canonical = f"{req.voice_id}|{combined}"
    content_hash = hashlib.sha256(canonical.encode("utf-8")).hexdigest()[:32]

    key = (
        f"stories/{_safe_path_segment(req.project_id)}/"
        f"{_safe_path_segment(req.pair_id)}/{content_hash}.mp3"
    )

    # Cache hit? Skip synthesis entirely.
    try:
        meta = _story_r2.head_object(key)
    except Exception as e:
        print(f"[finalize-story] R2 HEAD failed: {e}", flush=True)
        raise HTTPException(503, f"R2 head_object failed: {e}")
    if meta is not None:
        return FinalizeStoryResponse(
            url          = _story_r2.public_url(key),
    ...
    # Cache miss → synthesise the full narration in one edge-tts call.
    try:
        communicate = edge_tts.Communicate(combined, req.voice_id)
        audio = bytearray()
        async for chunk in communicate.stream():
            if chunk["type"] == "audio":
                audio += chunk["data"]
    except Exception as e:
        print(f"[finalize-story] edge-tts error: {e}", flush=True)
        raise HTTPException(503, f"TTS synthesis failed: {e}")
    if not audio:
        raise HTTPException(503, "TTS synthesis returned no audio data.")

    mp3_bytes = bytes(audio)
    try:
        _story_r2.put_object(key, mp3_bytes, content_type="audio/mpeg")
    except Exception as e:
        print(f"[finalize-story] R2 upload failed: {e}", flush=True)
        raise HTTPException(503, f"R2 upload failed: {e}")
